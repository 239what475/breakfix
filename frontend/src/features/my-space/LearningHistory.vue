  ...
  <section class="space-section learning-history" aria-labelledby="learning-history-title">
    ...
    <div v-else-if="items.length" class="space-row-list history-list">
      <article v-for="item in items" :key="`${item.challenge.id}-${item.ready_at}`" class="space-row history-row">
        ...
        <div class="space-row-main"><div class="space-row-title"><h3>{{ item.challenge.title }}</h3><span class="difficulty" :class="item.challenge.difficulty">{{ item.challenge.difficulty }}</span></div><p>{{ stateLabel(item) }} · {{ date(item.state === "completed" ? item.completed_at ?? item.ready_at : item.ready_at) }}</p></div>
