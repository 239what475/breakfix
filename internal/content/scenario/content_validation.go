...
func validateSolutionMarkers(scenario *Entry, solution []byte) error {
	counts := make(map[string]int, len(scenario.Checkpoints))
...
func validateMarkdownLinks(root, relative string) error {
	path, err := safeScenarioPath(root, relative)
	if err != nil {
...
func validateMarkdownLink(root, source, destination string) error {
	...
	path, err := safeScenarioPath(root, filepath.Join(filepath.Dir(source), resource))
	if err != nil {
