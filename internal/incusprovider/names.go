...
func NamesForBuildAttempt(prefix, workItemID string, attempt int64) (BuildNames, error) {
	if strings.TrimSpace(prefix) == "" || strings.TrimSpace(workItemID) == "" || attempt < 1 {
		return BuildNames{}, fmt.Errorf("%w: name prefix, work item ID, and positive attempt are required", ErrInvalid)
	}
	suffix := opaqueSuffix(fmt.Sprintf("%s\x00%d", workItemID, attempt), 24)
	return BuildNames{
