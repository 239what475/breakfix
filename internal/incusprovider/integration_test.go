...
func TestPreflightAgainstIncus(t *testing.T) {
	...
	for _, role := range []incusprovider.Role{
		...
		t.Run(string(role), func(t *testing.T) {
			...
			if result.ServerVersion != incusprovider.SupportedServerVersion {
...
func TestNodeImageAndEnvironmentAgainstIncus(t *testing.T) {
	...
	controllerClient, err := incusprovider.Connect(ctx, config)
	if err != nil {
	...
	environments := make([]incusprovider.ProvisionNodeEnvironmentRequest, 0, 2)
	for index := 0; index < 2; index++ {
		t.Logf("provision isolated Node environment %d", index+1)
		environmentUID := fmt.Sprintf("%s-environment-%d", runID, index)
		identity, err := incusprovider.IdentityForNodeEnvironment(config.NamePrefix, environmentUID, []string{"node"})
		if err != nil {
			t.Fatalf("derive environment identity: %v", err)
		}
		request := incusprovider.ProvisionNodeEnvironmentRequest{
	...
	result, err := controllerClient.ExecNode(ctx, incusprovider.ExecNodeRequest{
...
func TestNodeReverseProxyAgainstIncus(t *testing.T) {
	...
	files, err := incusprovider.ImageFilesFromDirectory(nodeReverseProxyFixture(t))
	if err != nil {
	...
	identity, err := incusprovider.IdentityForNodeEnvironment(config.NamePrefix, runID+"-environment", []string{"client", "proxy", "app"})
	if err != nil {
		t.Fatalf("derive environment identity: %v", err)
	}
	request := incusprovider.ProvisionNodeEnvironmentRequest{
...
func expectNodeCheck(t *testing.T, ctx context.Context, client *incusprovider.Client, request incusprovider.ProvisionNodeEnvironmentRequest, node, checkID string, passed bool) {
	t.Helper()
	result, err := client.ExecNode(ctx, incusprovider.ExecNodeRequest{
...
func logNodeDiagnostics(t *testing.T, ctx context.Context, client *incusprovider.Client, request incusprovider.ProvisionNodeEnvironmentRequest) {
	t.Helper()
...
func executeNodeAnswers(t *testing.T, ctx context.Context, client *incusprovider.Client, request incusprovider.ProvisionNodeEnvironmentRequest) {
	t.Helper()
	results := make(chan struct {
		node   string
		result incusprovider.ExecNodeResult
		err    error
...
func integrationConfig(t *testing.T, endpoint string, role incusprovider.Role) incusprovider.Config {
	t.Helper()
	tlsDir := requireEnv(t, "BREAKFIX_INCUS_TEST_TLS_DIR")
	roleDir := filepath.Join(tlsDir, string(role))
	return incusprovider.Config{
		Endpoint: endpoint,
...
func waitForNodeEnvironment(t *testing.T, ctx context.Context, client *incusprovider.Client, request incusprovider.ProvisionNodeEnvironmentRequest) {
	t.Helper()
...
func cleanupEnvironment(t *testing.T, client *incusprovider.Client, request incusprovider.ProvisionNodeEnvironmentRequest) {
	t.Helper()
...
func cleanupPublishedImage(t *testing.T, client *incusprovider.Client, candidateID string, result incusprovider.PublishNodeImageResult) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
