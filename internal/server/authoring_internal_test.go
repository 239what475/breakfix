...
func TestInternalAuthoringEndpointsFenceAttemptsAndFinalizeAtomically(t *testing.T) {
	...
	postInternalAuthoring(t, router, "context", *first, first.Credential(), http.StatusOK)
	now := time.Now().UTC()
	...
	postInternalAuthoring(t, router, "context", *first, first.Credential(), http.StatusConflict)

	plan := authoring.Plan{
		Metadata:    authoring.Metadata{Title: "Repair a service", Description: "Repair the service and restore its health endpoint.", Difficulty: "easy", Runtime: "node"},
	...
	postInternalAuthoring(t, router, "stage", *second, internalAuthoringStageRequest{
		LeaseCredential: second.Credential(),
		StageRevision:   0,
		Plan:            plan,
	...
	if stored.Status != agentruntime.RunSucceeded {
