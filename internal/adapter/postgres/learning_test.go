...
func TestTerminalUsageSessionDeduplicatesConnections(t *testing.T) {
	...
	if err := database.Environment.OpenTerminalConnection(ctx, second); err != nil {
		t.Fatal(err)
	}
	...
	last, err := database.Environment.CloseTerminalConnection(ctx, first.ID, started.Add(10*time.Second))
	if err != nil {
		t.Fatal(err)
	}
	if last {
	...
	last, err = database.Environment.CloseTerminalConnection(ctx, second.ID, started.Add(20*time.Second))
	if err != nil {
		t.Fatal(err)
	}
	if !last {
	...
	closed, err := database.Environment.FinishTerminalUsageSession(ctx, first.EnvironmentUID, started.Add(25*time.Second))
	if err != nil {
		t.Fatal(err)
	}
	if !closed {
...
func TestCleanupTerminalActivityClosesStaleConnectionAndSession(t *testing.T) {
	...
	if err := database.Environment.DeleteClosedTerminalConnections(ctx, started.Add(5*time.Minute)); err != nil {
		t.Fatal(err)
	}
...
func TestLearningSummaryAndHistoryUseUsageSessions(t *testing.T) {
	...
	summary, err := database.Environment.LearningSummary(ctx, "u-one", started.Add(3*time.Minute))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLearningHistoryFiltersAndPaginatesSameTimestampAttempts(t *testing.T) {
	...
	second, err := database.Environment.ListLearningHistory(ctx, "u-one", filter, 2, &LearningHistoryCursor{ReadyAt: first[1].ReadyAt, EnvironmentUID: first[1].EnvironmentUID}, readyAt.Add(2*time.Minute))
	if err != nil {
		t.Fatal(err)
	}
...
func newLearningTestDB(t *testing.T) *Store {
	t.Helper()
	return newTestDB(t)
}
