...
func scanPersistedScenarioRevision(row scanner) (*scenariodomain.Revision, error) {
	...
	if err := decodeOptionalJSON(artifact, &value.Artifact); err != nil {
		return nil, fmt.Errorf("decode scenario revision artifact: %w", err)
	}
...
func scanActiveScenarioRevision(row scanner) (*scenariodomain.ActiveRevision, error) {
	...
	if err := decodeOptionalJSON(artifact, &value.Revision.Artifact); err != nil {
		return nil, fmt.Errorf("decode active scenario revision artifact: %w", err)
	}
...
func insertPersistedScenarioRevisionTx(ctx context.Context, tx *Tx, value scenariodomain.Revision) error {
	...
	if err != nil {
		return fmt.Errorf("encode scenario revision artifact: %w", err)
	}
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO scenario_revisions
		(id, scenario_id, source_kind, source_ref, source_revision_id, base_active_revision_id, title, runtime,
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?::jsonb, ?, ?, ?, ?, ?::jsonb, ?, ?, ?)`, value.ID, value.ScenarioID, value.SourceKind,
...
func scenarioRevisionFromPublication(publicationTitle, runtime string, scenarioType content.ScenarioType, tags []string, contentRevision, materializedRevision string, publicationArtifact execution.ArtifactReference,
	...
	return scenariodomain.Revision{
		...
		Artifact: publicationArtifact, State: scenariodomain.RevisionActive, PublishedAt: publishedAt.UTC(), CreatedAt: publishedAt.UTC(),
	}
}
