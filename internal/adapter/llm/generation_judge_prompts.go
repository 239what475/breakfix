...
1. 资产完整性：公共文档、每个 hint，以及 runtime=node 的全部节点脚本或 runtime=k8s 的脚本是否齐全；operations-scenario 的每个 evidence 执行位置是否有 reproduce.sh；是否混入另一运行时目录，或把平台构建、入口、隐藏判定资产写进场景目录。
2. 现场一致性：现场说明、解答、提示、所有 generate.sh、所有 reproduce.sh、所有 answer.sh、versions、topology、initialization、目标现象、evidence 和每个检查点是否描述同一套故障与修复后状态；节点拓扑、evidence 和 checkpoint 的执行节点是否合理。
3. 判定模型：所有公开检查点通过是否是唯一完成条件；是否存在 verify.sh、隐藏条件、命令路径判定，或会写入环境、执行 answer.sh 的检查器。
4. 复现和检查点协议：每个无参数 reproduce.sh 是否显然会以唯一 JSON 文档报告其执行位置的全部 evidence id，且仅观察初始化后的目标现象；每个 checks.sh 是否显然会以唯一 JSON 文档报告其执行位置的全部 checkpoint id，失败是否保留诊断，stdout 是否只会有这一份 JSON；两者都不能执行、source 或触发 generate.sh、answer.sh 或用户修复脚本。
5. 运行时契约：Node 的每节点脚本是否只操作自身节点、是否误用 Kubernetes 或底层 Provider API、是否错误依赖并行脚本顺序；K8s 脚本是否只操作目标集群；是否把平台构建或入口职责写入题目资产。
6. 运行时正确性：必须静态推演全部 generate.sh 完成后的状态、每个 reproduce.sh 对初始现象的实际判断、全部 answer.sh 的副作用和每个检查点的实际判断。初始化必须构造可复现的明确现象，reproduce.sh 必须证明 manifest 所述证据；标准答案必须针对同一事实修复并使所有检查点要求的最终状态存在。创建一个用户可执行脚本但不执行它，不是对该最终状态的修复；文件写入后的时间戳、权限等元数据也必须与检查器和现场说明一致。K8s 检查器不得依赖 kubectl run、外部探测镜像或不稳定的 Pod 文本匹配。
