...
type conversation struct {
	...
	emit    func(assistant.StreamEvent)

	mu        sync.Mutex
	evidence_ []assistant.Evidence
}
...
func (c *conversation) notify(event assistant.StreamEvent) {
	if c.emit != nil {
		c.emit(event)
	}
}
...
func (c *conversation) record(kind, label string) {
	...
	c.evidence_ = append(c.evidence_, assistant.Evidence{Kind: kind, Label: label})
}

func (c *conversation) evidence() []assistant.Evidence {
	c.mu.Lock()
	defer c.mu.Unlock()
	return append([]assistant.Evidence{}, c.evidence_...)
}
...
func terminalWindowOpen(terminals []assistant.TerminalContext, node, window string) bool {
