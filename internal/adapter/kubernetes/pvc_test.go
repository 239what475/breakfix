...
func TestEnsureWorkspacePVCCreatesAndChecksOwnership(t *testing.T) {
	...
	if err := client.EnsureWorkspacePVC(ctx, "opensandbox", "workspace-one", "workflow-one", "1Gi"); err != nil {
	...
	if err := client.EnsureWorkspacePVC(ctx, "opensandbox", "workspace-one", "workflow-two", "1Gi"); err == nil {
