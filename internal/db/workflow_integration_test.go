...
func TestGenerationWorkflowPersistsRepairAndPublicationLifecycle(t *testing.T) {
	...
	candidateOne := finalizeGeneratedCandidate(t, database, claim, sessionID, 1, now)
	...
	if err := database.FinalizeGenerationJudgement(ctx, claim, judgeRun.ID, false, "检查点描述与实际题目不一致", now); err != nil {
	...
	candidateTwo := finalizeGeneratedCandidate(t, database, claim, sessionID, 2, now)
	claim = refreshGenerationClaim(t, database, claim, now)
	approveGenerationJudgement(t, database, claim, now)
	claim = refreshGenerationClaim(t, database, claim, now)
	advanceGenerationBuildAndArtifact(t, database, claim, now)
	claim = refreshGenerationClaim(t, database, claim, now)
	if err := database.RecordGenerationVerificationEnvironment(ctx, claim, verificationEnvironment(claim), now); err != nil {
		t.Fatalf("record failed candidate verification environment: %v", err)
	}
	if err := database.CompleteGenerationVerification(ctx, claim, verificationReport(false), now); err != nil {
	...
	firstVerifiedCandidate := finalizeGeneratedCandidate(t, database, claim, sessionID, 3, now)
	claim = refreshGenerationClaim(t, database, claim, now)
	approveGenerationJudgement(t, database, claim, now)
	claim = refreshGenerationClaim(t, database, claim, now)
	advanceGenerationBuildAndArtifact(t, database, claim, now)
	...
	if err := database.CompleteGenerationVerification(ctx, claim, verificationReport(true), now); err != nil {
		t.Fatalf("record passing candidate verification: %v", err)
	}
	paused, err := database.GetGenerationWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	resumed, err := database.ResumeGenerationForRevision(ctx, sessionID, userID, 1, resumedAt)
	if err != nil {
	...
	finalCandidate := finalizeGeneratedCandidate(t, database, claim, sessionID, 4, resumedAt)
	...
	claim = refreshGenerationClaim(t, database, claim, resumedAt)
	if err := database.RecordGenerationVerificationEnvironment(ctx, claim, verificationEnvironment(claim), resumedAt); err != nil {
		t.Fatalf("record reverified candidate environment: %v", err)
	}
	if err := database.CompleteGenerationVerification(ctx, claim, verificationReport(true), resumedAt); err != nil {
	...
	if err := database.CompleteGenerationChallengePublish(ctx, claim, artifactReference(), challengeID, workflowTestDigest, "", publicationTime); err != nil {
	...
	if err := database.CompleteGenerationCleanup(ctx, claim, publicationTime); err != nil {
		t.Fatalf("complete generation cleanup: %v", err)
	}
	completed, err := database.GetGenerationWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	if _, err := database.GetTaxonomyWorkflowByChallenge(ctx, challengeID, workflowTestDigest); err != nil {
...
func TestGenerationWorkflowReclaimsInfrastructureRetryAndCleansTerminalStates(t *testing.T) {
	...
	claim := claimGenerationWorkflow(t, database, workflow.ID, "generator-a", now)
	failed, err := database.ReportGenerationInfrastructureFailure(ctx, claim, generation.StateGenerating, generation.Failure{
	...
	if err := database.RecoverExpiredGenerationWorkflows(ctx, *takeover.Workflow.DeadlineAt); err != nil {
		t.Fatalf("expire generation workflow: %v", err)
	}
	expired, err := database.GetGenerationWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	if err := database.CompleteGenerationCleanup(ctx, cleanup, *takeover.Workflow.DeadlineAt); err != nil {
		t.Fatalf("complete failed cleanup: %v", err)
	}
	terminal, err := database.GetGenerationWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	if err := database.CancelGenerationWorkflow(ctx, cancelledWorkflow.ID, "author discarded this draft", now.Add(time.Hour)); err != nil {
	...
	if err := database.CompleteGenerationCleanup(ctx, cancelCleanup, now.Add(time.Hour)); err != nil {
		t.Fatalf("complete cancelled cleanup: %v", err)
	}
	cancelled, err := database.GetGenerationWorkflow(ctx, cancelledWorkflow.ID)
	if err != nil {
...
func TestGenerationLeaseRenewsAfterSuccessfulPhaseResetsAttempt(t *testing.T) {
	...
	workflow, sessionID, _ := createGenerationWorkflowFixture(t, database, now)

	initial := claimGenerationWorkflow(t, database, workflow.ID, "generate-a", now)
	retried, err := database.ReportGenerationInfrastructureFailure(ctx, initial, generation.StateGenerating, generation.Failure{
	...
	finalizeGeneratedCandidate(t, database, claim, sessionID, 1, retried.NextRunAt)
	advanced, err := database.GetGenerationWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	if err := database.RenewGenerationLease(ctx, claim, time.Minute, retried.NextRunAt.Add(time.Second)); err != nil {
		t.Fatalf("renew generation lease after phase transition: %v", err)
	}
	renewed, err := database.GetGenerationWorkflow(ctx, workflow.ID)
	if err != nil {
...
func TestTaxonomyWorkflowPersistsCommitteeRoundsAndReclaimsTechnicalFailures(t *testing.T) {
	...
	workflow, _, err := database.CreateOrGetTaxonomyWorkflow(ctx, challengeID, challengeRevision, "base-0", now)
	if err != nil {
	...
	rejected, err := database.GetTaxonomyWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	if err := database.SetTaxonomyExpectedSnapshot(ctx, claim, "sha256:"+strings.Repeat("c", 64), now); err != nil {
		t.Fatalf("record expected taxonomy snapshot: %v", err)
	}
	if err := database.CompleteTaxonomyPublication(ctx, claim, "sha256:"+strings.Repeat("c", 64), now); err != nil {
		t.Fatalf("complete taxonomy publication: %v", err)
	}

	retryID := "chal-taxonomy-retry"
	retryRevision := "sha256:" + strings.Repeat("d", 64)
	retryWorkflow, _, err := database.CreateOrGetTaxonomyWorkflow(ctx, retryID, retryRevision, "base-0", now)
	if err != nil {
...
func TestTaxonomyLeaseRenewsAfterSuccessfulPhaseResetsAttempt(t *testing.T) {
	...
	advanced, err := database.GetTaxonomyWorkflow(ctx, workflow.ID)
	if err != nil {
	...
	oldExpiry := *advanced.LeaseExpiresAt
	if err := database.RenewTaxonomyLease(ctx, claim, time.Minute, now.Add(time.Second)); err != nil {
		t.Fatalf("renew taxonomy lease after phase transition: %v", err)
	}
	renewed, err := database.GetTaxonomyWorkflow(ctx, workflow.ID)
	if err != nil {
...
func createGenerationWorkflowFixture(t *testing.T, database *DB, now time.Time) (*generation.Workflow, string, string) {
	t.Helper()
	ctx := context.Background()
	userID := authoring.NewID("workflow-user")
	if _, err := database.CreateUserWithAuth(userID, userID, "", ""); err != nil {
		t.Fatalf("create workflow author: %v", err)
	}
	plan := generationTestPlan()
	session, err := database.CreateAuthoringSession(ctx, authoring.Session{ID: authoring.NewID("authoring"), UserID: userID}, plan)
	if err != nil {
		t.Fatalf("create authoring session: %v", err)
	}
	if _, err := database.ReplaceAuthoringPlan(ctx, session.ID, session.UserID, 0, plan, authoring.StateIntentReview); err != nil {
		t.Fatalf("confirm authoring plan: %v", err)
	}
	workflow, err := database.CreateGenerationWorkflow(ctx, session.ID, session.UserID, 1, now)
	if err != nil {
...
func claimGenerationWorkflow(t *testing.T, database *DB, workflowID, workerID string, now time.Time) generation.Claim {
	t.Helper()
	claim, err := database.ClaimGenerationWorkflow(context.Background(), workerID, time.Minute, now)
	if err != nil {
...
func refreshGenerationClaim(t *testing.T, database *DB, claim generation.Claim, now time.Time) generation.Claim {
	t.Helper()
	refreshed, err := database.RefreshGenerationClaim(context.Background(), claim.Workflow.ID, claim.LeaseOwner, now)
	if err != nil {
...
func finalizeGeneratedCandidate(t *testing.T, database *DB, claim generation.Claim, sessionID string, sequence int, now time.Time) candidate.Revision {
	...
	if err := database.FinalizeGeneratedCandidate(context.Background(), claim, run.ID, revision, now); err != nil {
		t.Fatalf("finalize generated candidate %d: %v", sequence, err)
	}
	return revision
}

func approveGenerationJudgement(t *testing.T, database *DB, claim generation.Claim, now time.Time) {
	t.Helper()
	run := startGenerationRun(t, database, claim, generator.JudgePurpose, now)
	if err := database.FinalizeGenerationJudgement(context.Background(), claim, run.ID, true, "", now); err != nil {
...
func advanceGenerationBuildAndArtifact(t *testing.T, database *DB, claim generation.Claim, now time.Time) {
	...
	claim = refreshGenerationClaim(t, database, claim, now)
	if err := database.CompleteGenerationArtifactPublish(context.Background(), claim, artifactReference(), now); err != nil {
...
func claimTaxonomyWorkflow(t *testing.T, database *DB, workflowID, workerID string, now time.Time) taxonomy.Claim {
	t.Helper()
	claim, err := database.ClaimTaxonomyWorkflow(context.Background(), workerID, time.Minute, now)
	if err != nil {
...
func refreshTaxonomyClaim(t *testing.T, database *DB, claim taxonomy.Claim, now time.Time) taxonomy.Claim {
	t.Helper()
	refreshed, err := database.RefreshTaxonomyClaim(context.Background(), claim.Workflow.ID, claim.LeaseOwner, now)
	if err != nil {
