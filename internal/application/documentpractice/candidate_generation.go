...
func (b CandidateBlueprint) Validate(plan domain.LearningUnitPlan, profile runnable.RuntimeProfile, lifecycle runnable.LifecyclePolicy) error {
...
func CompileCandidate(plan domain.LearningUnitPlan, profile runnable.RuntimeProfile, lifecycle runnable.LifecyclePolicy, blueprint CandidateBlueprint, now time.Time) (domain.PracticeCandidate, []byte, error) {
	if now.IsZero() {
		return domain.PracticeCandidate{}, nil, errors.New("candidate compilation time is required")
	}
	if err := blueprint.Validate(plan, profile, lifecycle); err != nil {
