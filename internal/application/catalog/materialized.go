...
// revisions referenced by one Roadmap revision. Directories not referenced by
// the revision are intentionally ignored: they are valid historical revisions
// and materialize-before-publish results, neither of which may alter current
// catalog reads.
func materializedChallengeIndex(revision roadmapdomain.Revision, challengesDir string) (map[string]challenge.Entry, error) {
	...
	for _, binding := range revision.ChallengeBindings {
		path := challenge.MaterializedPath(binding.Challenge.SourceSlug, binding.Challenge.RevisionID)
		if path == "" {
			return nil, materializedIntegrity(binding.Challenge, "materialized source path is invalid")
		}
		entry, err := challenge.ValidateDir(filepath.Join(challengesDir, filepath.FromSlash(path)))
		if err != nil {
		...
		if entry.RevisionID != binding.Challenge.RevisionID {
			return nil, materializedIntegrity(binding.Challenge, "materialized revision_id is %q", entry.RevisionID)
		}
