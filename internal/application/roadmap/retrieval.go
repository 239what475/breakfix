...
func (r FilesystemChallengeReader) ReadRoadmapChallenge(_ context.Context, binding domain.ChallengeBinding) (ChallengeContent, error) {
	...
	if path == "" {
		return ChallengeContent{}, fmt.Errorf("roadmap challenge %q has an invalid revision path", binding.Challenge.ID)
	}
	entry, err := challenge.ValidateDir(filepath.Join(r.Root, filepath.FromSlash(path)))
	if err != nil {
		return ChallengeContent{}, fmt.Errorf("read roadmap challenge %q: %w", binding.Challenge.ID, err)
	}
	if entry.ID != binding.Challenge.ID || entry.RevisionID != binding.Challenge.RevisionID || entry.Title != binding.Challenge.Title || entry.ContentRevision != binding.Challenge.ContentRevision ||
