...
func TestRealPageFixtureMatchesGoldenAcrossRunsAndWorkers(t *testing.T) {
	...
	for _, workers := range []int{1, 8} {
		...
		if err := Run(Config{Root: root, Out: out, Workers: workers, Version: "docs-project-v10", Pages: fixturePages, SiteOrigin: "https://kubernetes.io"}); err != nil {
			t.Fatalf("workers %d: %v", workers, err)
		}
		assertDirectoriesEqual(t, golden, out)
	}
	repeat := filepath.Join(t.TempDir(), "documents")
	if err := Run(Config{Root: root, Out: repeat, Workers: 8, Version: "docs-project-v10", Pages: fixturePages, SiteOrigin: "https://kubernetes.io"}); err != nil {
		t.Fatal(err)
	}
...
func TestRealPageFixtureResumeRebuildsOnlyMissingPairs(t *testing.T) {
	...
	golden := filepath.Join(root, "golden")
	out := filepath.Join(t.TempDir(), "documents")
	config := Config{Root: root, Out: out, Workers: 8, Version: "docs-project-v10", Pages: fixturePages, SiteOrigin: "https://kubernetes.io"}
