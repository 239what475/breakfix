...
func TestRuntimeArtifactOwnershipAcceptsActionScopedReferences(t *testing.T) {
	...
	staging := execution.ArtifactReference{Runtime: scenario.RuntimeK8s, OCIReference: stagingRepository + "@sha256:" + fingerprint}
	k8sAction := runtime.Context{Identity: runtime.Identity{CandidateID: "candidate-k8s"}, Snapshot: execution.Snapshot{Runtime: scenario.RuntimeK8s}}
	if err := handler.validateRuntimeStagingArtifact(k8sAction, staging); err != nil {
		t.Fatalf("validate K8s staging artifact: %v", err)
	}
	finalRepository, err := candidate.ScenarioOCIRepository(handler.registryRepository, "scenario-k8s", "chrev-aaaaaaaaaaaaaaaa")
	if err != nil {
		t.Fatal(err)
	}
	...
	nodeAction := runtime.Context{Identity: runtime.Identity{CandidateID: "candidate-node"}, Snapshot: execution.Snapshot{Runtime: scenario.RuntimeNode},
		Build: &execution.BuildOutput{Runtime: scenario.RuntimeNode, Incus: &execution.IncusBuildReference{Fingerprint: fingerprint}}}
	nodeStaging := execution.ArtifactReference{Runtime: scenario.RuntimeNode, IncusAlias: candidateAlias, IncusFingerprint: fingerprint}
