...
func TestCandidateAndChallengeOCIReferencesUseDistinctOpaqueScopes(t *testing.T) {
	...
	candidateRef, err := CandidateOCIImageReference("registry.breakfix.internal/breakfix", candidateID)
	if err != nil {
		t.Fatal(err)
	}
	challengeRef, err := ChallengeOCIImageReference("registry.breakfix.internal/breakfix", challengeID)
	if err != nil {
		t.Fatal(err)
	}
	if candidateRef != "registry.breakfix.internal/breakfix/candidates/"+OpaqueName(candidateID)+":artifact" {
		t.Fatalf("candidate image reference = %q", candidateRef)
	}
	if challengeRef != "registry.breakfix.internal/breakfix/challenges/"+OpaqueName(challengeID)+":published" {
...
func TestOCIReferencePartsPreserveImmutableIdentity(t *testing.T) {
	const reference = "registry.breakfix.internal/breakfix/candidates/abc@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	repository, err := OCIRepository(reference)
	if err != nil {
		t.Fatal(err)
	}
	if repository != "registry.breakfix.internal/breakfix/candidates/abc" {
