...
将当前“左侧窄题目列表 + 右侧选中题目摘要”的首页替换为独立的全宽 Catalog。目录是用户进入平台后的主入口，用于浏览、筛选、比较并开始挑战；工作台只承担已经开始的挑战，不能再承担题库浏览和题目详情预览。

Catalog 的信息密度和结构参考 iximiuz 的筛选栏与挑战列表：固定的筛选区域、可快速扫描的主列表、明确而克制的唯一操作。它不复制作者、收藏、提交次数、社区内容或全局导航等本项目当前没有产品意义的数据。

<img src="image-2.png" alt="iximiuz 的筛选式挑战目录参考" width="100%">

### 产品原则

1. 目录中的题目元数据只来自公开 `GET /api/challenges` 返回的 `title`、`description`、`difficulty`、`tags`、`runtime` 和 `published_at`；登录用户的 `active`、实时 checkpoint `progress` 与历史 `solved` 由 Gateway 合并当前环境和用户完成记录得出。首版不增加 catalog 数据库、静态 `catalog.yaml` 或详情接口。
2. 目录不保留“当前选中题目”的右侧摘要面板。题目卡片本身提供足够的判断信息，唯一会改变状态的动作是 `Start challenge`。
3. 同一个 `Start challenge` 请求保持幂等：无环境时创建环境，已有 Ready 或 Draining 环境时恢复环境。卡片可以显示“进行中”状态，但不能分裂为 Start、Resume 等多个主按钮。
4. 难度、标签、运行时与完成状态均是筛选条件，不是只读的小字。筛选结果必须立即反映在列表数量和卡片上。
5. 首版在已加载的 catalog 数据上做前端过滤和搜索；不能为了少量题目先引入服务端查询语言、分页协议或第二份索引。
6. 保持单页应用状态模型，不引入 `vue-router`。开始题目后切换到现有 `ChallengeWorkspace`；从工作台返回时恢复当前目录筛选与滚动位置。
7. 首版不限制用户同时进行中的挑战数。单个浏览器标签一次只展示一个工作台，但用户可以返回目录继续开始其他题目，或在多个浏览器标签中并行操作；资源配额、排队与更严格的回收策略留待真实容量需求出现后单独设计。

### 信息架构与交互

目录固定占满应用视口。顶栏只保留品牌、登录状态与作者入口；主体由筛选栏和挑战列表组成，页面本身不产生横向溢出，筛选栏和列表在内容超过视口时各自滚动。

左侧筛选栏：

- 顶部提供标题、搜索框、匹配结果数和 `Reset filters`。
- `Difficulty`：Easy、Medium、Hard，多选。
- `Runtime`：Container、VCluster，多选。
- `Tags`：从当前 catalog 的全部标签动态聚合并按字母稳定排序，多选；不能维护另一份手写分类树。
- `Status`：Todo、In progress、Completed。它们按优先级分别映射为 `!solved && !active`、`active`、`solved && !active`；已经完成的题被再次开始时显示为 In progress，并可保留轻量的历史完成标识。未登录用户不显示个人状态筛选，也不将所有题错误标为 Todo。
- 同一筛选组内的多个选项使用“满足任一项”，不同筛选组之间使用“同时满足”；搜索匹配标题、简介和标签。

右侧挑战列表：

- 列表工具栏显示当前匹配结果数，并提供 `Newest first` 与 `Oldest first` 排序，默认按 `published_at` 倒序；排序只改变卡片顺序，不影响任何环境状态。
- 使用纵向、高信息密度的挑战卡片，而不是窄侧栏条目或大幅营销式卡片。
- 每张卡片从上到下展示运行时与难度、标题、最多两行简介、标签，以及状态提示。进行中的卡片显示当前环境的 `已完成检查点数 / 总检查点数`；标签用于扫描和比较，不能挤占标题或使卡片高度不稳定。
- 右下角只保留 `Start challenge`。点击后显示卡片内加载状态，成功后直接进入对应工作台；未登录时先进入既有登录流程，登录成功后继续该题的开始动作。
...
这次改造应替换现有 `ChallengeCatalog` 的侧栏职责，并移除由 `selectedId` 驱动的 catalog 详情区域。目录拆分为清晰的组件边界：

- `ChallengeCatalogPage`：加载后的目录状态、筛选状态、开始挑战后的页面切换与滚动恢复。
- `CatalogFilters`：搜索、筛选、重置和窄屏抽屉。
- `ChallengeList` 与 `ChallengeCard`：只负责展示过滤结果和发出开始事件，不持有环境创建逻辑。

`useChallengeSession` 继续拥有挑战列表刷新与 `startChallenge` 的异步边界；筛选状态不进入 Gateway、不写入 challenge 文件系统，也不和工作台的环境状态混在一起。

### 发布元数据与默认策略

- Gateway 在作者发布已验证 revision 时写入不可变的 `published_at`，并将它保存在正式 `data/challenges/<id>/` 目录中的题目元数据。文件系统仍是 catalog 的唯一权威来源；`VerifyTask` 不写入或修改这个字段。
- `GET /api/challenges` 将 `published_at` 作为摘要字段返回，目录默认按其倒序展示新发布的挑战。现有预置题在迁移时补入一个稳定值，不能依赖部署时的文件修改时间。
- 登录后的默认目录展示全部挑战；`Todo`、`In progress` 和 `Completed` 仅是可选筛选，避免隐藏已完成或可恢复的题目。
- 首版完整展示当前 catalog 聚合出的所有标签；标签数量达到实际可用性阈值前，不增加“显示更多”或标签搜索。

### 用户完成记录

`Environment` 是一次可回收的运行时资源，不能作为用户长期学习进度的权威来源：检查点通过后，Controller 会将环境标记为 Completed，但空闲回收最终会删除对应 CRD。因此，Catalog 的 Completed 状态必须来自独立的用户数据。

- 新增 SQLite `user_challenge_progress`，以 `(user_id, challenge_id)` 为主键，至少保存首次 `completed_at` 和完成该记录的 `environment_uid`。它是用户账户数据，不进入 challenge 文件夹，也不参与题目验证或发布。
- Controller 在所有检查点首次通过时，以环境 `spec.userRef`、`spec.challengeRef`、环境 UID 和完成时间幂等写入完成记录；数据库写入失败时不能把环境标记为 Completed，必须让 reconciliation 重试。重复 reconcile 同一 UID 不得创建第二条记录或改变首次完成时间。
- Controller 只依赖一个窄的 CompletionRecorder 接口。当前 Gateway 与 Controller 同一二进制运行时注入 SQLite 实现；未来独立部署 Controller 时可替换为 Gateway 内部 API 或共享进度服务，而不改变 checkpoint 完成的所有权。
- Gateway 列出 Catalog 时读取当前用户的完成记录：`solved` 表示该用户曾完成该题，即使原环境已经回收；`active` 仍只表示当前存在 Ready 或 Draining 环境。未登录的公开 catalog 不返回用户进度语义。
- Gateway 同时从当前 active Environment 的 checkpoint status 计算 `progress`：`passed` 是已通过检查点数，`total` 是题目声明的检查点总数。它只对登录用户返回，并且只代表当前环境；尚未得到首轮检查结果时返回 `0 / total`，不以缺失状态伪造已完成进度。
- 不持久化未完成的检查点结果、终端输出或临时环境状态。它们只属于运行中的挑战环境，环境回收后重新开始题目即从初始状态开始；`user_challenge_progress` 只保留完成这一账户事实。
- 用户重新开始已经完成的题目时，完成记录保留；目录状态按 In progress 优先显示，完成历史不丢失。

完成记录的写入路径如下：

```text
Controller 检查点全部通过
  -> 幂等写入 user_challenge_progress
  -> 更新 Environment.status 为 Completed
  -> 环境依照原有 TTL 回收并删除 CRD
  -> Gateway 以用户完成记录继续返回 solved=true
```

### 验收标准

- 首次进入只请求 catalog 摘要，不请求任意题目的完整 problem、solution 或检查点内容。
- 所有现有难度、运行时、标签和登录后的状态字段均可组合筛选；重置后恢复完整列表。
- 一个进行中题目和一个已完成题目在状态筛选下的显示符合 `active`、`solved` API 语义。
- 进行中题目的卡片展示该环境当前真实的 `passed / total` checkpoint 进度；环境被重置、Stop 或回收后，这个临时进度不再显示，也不会污染历史完成记录。
- 用户完成一题后，即使该环境被 Stop 或被 TTL 回收并删除，重新加载 Catalog 仍将该题标为 Completed。
- 每张结果卡片只有一个 `Start challenge`；它使用既有开始接口进入工作台，不出现提交、预览详情或第二个环境生命周期按钮。
