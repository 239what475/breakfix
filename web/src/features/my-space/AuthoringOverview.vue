...
<template>
  <section class="space-section authoring-overview" aria-labelledby="authoring-title">
    <div class="space-section-heading"><div><p class="eyebrow">Create scenarios</p><h2 id="authoring-title">Authoring</h2></div><button class="text-button" type="button" @click="emit('authoring')">Open studio</button></div>
    ...
    <div v-if="authoring.published.length" class="published-grid">
      <article v-for="published in authoring.published" :key="published.scenario.id" class="published-card">
        ...
        <div class="published-actions">
          ...
          <button v-if="published.state === 'active'" class="text-button danger-text" type="button" @click="emit('deprecate', published.scenario.id)">Deprecate</button>
          <button v-if="published.state === 'active'" class="text-button" type="button" @click="emit('catalog', published.scenario.id)">View in catalog</button>
        </div>
      </article>
    </div>
    <p v-else class="space-empty">Published scenarios will appear here.</p>
  </section>
</template>
