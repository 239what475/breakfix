...
Verify 和 ChallengePublishing 由 Runtime Worker 执行。`NeedsAuthorReview` 是唯一的作者审核状态。作者确认当前 verified candidate 后，
workflow 直接进入 `ChallengePublishing`，不存在分类、课程归属或额外审核状态。
...
`ChallengePublishing` 的 Worker promotion 成功后，Server publication finalizer 校验 immutable artifact、materialize 场景 source，
并在同一数据库事务中写入 stable Challenge、其 active revision、candidate 发布事实、作者会话发布状态与 workflow `Published`。finalizer
的确定性错误使 workflow 失败；瞬时错误保留 `ChallengePublishing` 并使用持久化诊断和下一次重试时间。旧 revision、artifact、
Environment 与学习记录从不被 active pointer 覆盖。

弃用不属于 Generation state。生命周期 API 只将 stable Challenge 标为 `deprecated`，阻止新 Environment；历史内容不删除。新 Environment
...
Server 创建 commit intent，Worker 执行最终 artifact promotion，Server materialize source 并在一个事务中公开所有 stable Challenge 和
active revision。相同 digest 的重启或 lease 接管只恢复未完成阶段，不能重复分配 challenge identity 或重复已成功 promotion。
...
每个 Runtime Worker 进程独立运行 Action Loop 与 Reaper Loop：前者一次领取 Build、artifact publish、Verify 或 ChallengePublishing
