...
metadata:
  ...
  version: 2026.08.01
entries:
  - path: scenarios/service-recovery
...
revision、materialized source、artifact、Environment 和学习记录保留。Catalog Release 创建的 Scenario 是 immutable 基线，不能通过
作者入口修订或弃用。

Environment、checkpoint evidence 和学习 attempt 保存创建时的 `scenario_revision_id`。任何历史读取按
`Scenario.id + scenario_revision_id` 解析 durable revision，不能跟随 active pointer；当前 Catalog 只读取状态为 active 的 Scenario
及其 active revision。
...
每个 immutable Scenario revision 的目录为：

```text
data_dir/scenarios/<source_slug>/<scenario_revision_id>/
```
