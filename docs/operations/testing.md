...
这些入口需要显式环境变量和已部署的当前架构。Node/K8s 作者验收从自然语言题意开始，经过 Server 直接 Authoring 对话、Generate Worker、真实验证、作者确认和正式发布，再启动学习环境运行答案。它们是完整流程验收，必须串行运行并在失败时保留 Workflow、AgentRun、CandidateRevision、Environment 和 Worker 日志用于定位，不能自动重跑掩盖问题。
...
- 未部署当前 `breakfix-generate-worker` 时，不能将旧 Deployment 的结果视为本次架构验收。
