...
Publisher 才能推镜像、Verifier 在真实 Environment 上运行 answer/checkpoints。VerifyTask
已经是“题目资产的可信评测任务”，Environment checkpoint 是“学习者环境的持续完成事实”。
...
- **可重跑而不混淆结果的报告模型**：VerifyTask 的 report 应保留 attempt 编号、任务阶段、
  题目 revision、镜像 digest 和结构化结论。重跑创建新 attempt/Job，旧报告保持可读；不能
  用新日志覆写曾经验证过的结果。
...
- 当基础镜像、runtime init 或 checkpoint runner 发生平台级变更时，设计显式的
  re-verification campaign：对已发布 artifact revision 创建新的 VerifyTask，而不是悄悄
...
- 不以 chroot judgehost 替换 Kubernetes Job 与 NetworkPolicy 隔离。

## 结论

DOMjudge 验证了独立、可审计、可重跑评测执行者的价值，而 Breakfix 已有更贴合 Kubernetes
题目安全边界的实现。近期应加强 VerifyTask 报告的不可变性和可读性，不应引入竞赛模型。
