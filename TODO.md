...
统一 Agent Runtime 与 Runtime Worker 的迁移已经完成。当前 P0 不再保存该迁移的历史设计，而是处理架构审查确认的六个问题。实现应沿用现有 Server、Controller、PostgreSQL、Runtime Worker 与动态 Environment 边界；不引入新的 Deployment、通用队列、周期扫描器、完整性缓存或兼容路径。

### 1. 补齐 VK8sEnvironment 学习者网络隔离

**现状问题：** `VK8sEnvironment` 只配置了 ResourceQuota 和 LimitRange，没有启用 NetworkPolicy。学习者控制的 workload 可能访问宿主集群控制面、平台服务、私网和云 metadata；实际暴露范围取决于底层 CNI。

**目标行为：** 每个 VK8sEnvironment 默认形成与 NodeEnvironment 等价的网络安全边界。学习者 workload 只允许题目环境内部必要流量、DNS 和明确的公网出口，不能访问平台网络、宿主 Kubernetes API、Service/Pod CIDR、私网、link-local 或 metadata 地址。

**实现边界：**

- 启用 vcluster chart 原生的 `policies.networkPolicy`，由上游能力覆盖 vcluster control plane 和 workload，不在 Breakfix 中复制同一套策略模板。
- Breakfix 只补充 management terminal 的策略，以及部署环境特有的 Service CIDR、Pod CIDR、平台 namespace 和其他受保护 CIDR。
- 受保护网络必须来自显式部署配置，不能根据单个开发集群写死。
- Kind 只用于验证资源渲染；安全验收必须使用真正执行 NetworkPolicy 的 CNI。

**必须验证：** vcluster control plane、management terminal 和学习者 workload 的策略都被正确创建；题目内部通信、DNS 和允许的公网访问仍可用；从真实学习者 workload 无法访问平台接口、宿主 API、私网与 metadata。

### 2. 为调试路由建立独立安全边界

**现状问题：** 以下调试接口注册在公开 Router 上，既没有用户认证，也没有 Runtime Worker 认证：

- `POST /internal/debug/roadmap-maintenance`
- `GET /internal/debug/roadmap-revisions/:revision_id/export`

它们可以触发有模型成本的全局 Roadmap maintenance，并导出包含题解和检查脚本的非公开内容。

**目标行为：** 生产默认不存在这些路由。仅在部署者显式开启远程调试时注册，并要求独立的 debug credential。

**实现边界：**

- 调试 credential 不得复用用户 JWT、Runtime Worker identity 或其他业务身份。
- 路由注册、认证中间件和部署配置必须共同受同一个显式开关控制。
- 不新增独立调试服务或 Deployment；关闭开关时公开 Router 中不能留下可调用入口。

**必须验证：** 默认部署不注册调试路由；显式开启后只有正确的 debug credential 可以调用；用户和 Runtime Worker 凭据不能获得调试权限；导出与 maintenance 的正常调试能力保持可用。

### 3. 建立已发布 Catalog 的物化完整性契约

**现状问题：** Catalog 读取先枚举 Server PVC 上存在的目录，再投影当前 Roadmap。整个 PVC 或单个题目目录丢失时，Roadmap binding 会被静默过滤，`/readyz` 仍可能成功并把损坏的题库报告为空题库。

**目标行为：** 当前 Roadmap 中每个 Challenge binding 都必须对应一份内容、元数据和文件权限完全匹配的 materialized source；缺失或不匹配必须返回明确的 Catalog integrity error，不能静默隐藏。

**实现边界：**

- 发布物化时，以规范化并排序后的文件路径、文件内容和可执行位计算 `materialized_revision`。
- 将 `materialized_revision` 写入运行时 Roadmap challenge binding。它是平台完整性字段，不导出到 portable Catalog Release；`content_revision` 继续作为可移植的源内容身份。
- 读取时先按 Challenge ID 建立物化文件索引，再严格遍历当前 Roadmap bindings，核对 `id`、`title`、`content_revision`、`source_slug` 和 `materialized_revision`。
- 公开 Catalog 读取仍先经过 configured release availability gate。`/readyz` 复用相同完整性逻辑并使用短超时，但绕过该 availability gate，避免初始化期间形成 Server 与 Runtime Worker 的启动死锁。
- 该检查自然同时覆盖 PostgreSQL 中的 Roadmap 与 Server data PVC，不增加独立 ping、周期扫描器或缓存。
- Roadmap 未引用的额外目录在“先物化、后提交 Roadmap”的正常窗口内是合法的，不影响 readiness；超过宽限期后由既有诊断和回收入口处理。

**必须验证：** PVC 根目录缺失、被引用题目缺失、内容变化、路径变化或可执行位变化都会使 Catalog 和 readiness 报完整性错误；额外未引用目录不阻断发布窗口；空平台仍能通过 bootstrap readiness 完成首个 Catalog Release；portable release 不包含 `materialized_revision`。

### 4. 将 Runtime Reaper 变成独立后台循环

**现状问题：** Runtime Worker 只有在普通 Build、Publish 或 Verify action 为空时才调用 `reapOne`。只要 action 持续存在，verification Environment、临时构建镜像和未发布 artifact 的后台清理就可能长期饥饿；固定先查 Generation 再查 Catalog 也会造成 scope 饥饿。

**目标行为：** 清理始终在后台自动推进，不参与普通 action 的先后顺序，也不阻塞业务状态；Generation 与 Catalog 在 action 和 cleanup 两侧都不能互相饿死。

**实现边界：**

- 保持唯一的 Runtime Worker Deployment；每个 Worker 进程同时运行一个 `Action Loop` 和一个 `Reaper Loop`。
- `Action Loop` 只领取 Build、ArtifactPublishing、Verifying 和 ChallengePublishing；`Reaper Loop` 只领取现有的幂等资源清理任务。
- 两个循环继续使用现有 lease 和围栏，各自一次只处理一个任务，并在进程退出时独立、可控地停止。
- Server 的 action claim 与 cleanup claim 都在 Generation 和 Catalog 之间轮换；当前一侧为空时立即回退到另一侧。
- 不增加清理队列、Deployment、workflow state 或通用调度框架。

**必须验证：** 持续存在普通 action 时 cleanup 仍会推进；Generation 或 Catalog 单侧持续积压时另一侧仍能被领取；两个循环的 lease 接管、迟到结果拒绝和幂等回收契约保持成立。

### 5. 修正 Roadmap 的 AgentRun 语义

**现状问题：** Roadmap 当前把一次技术失败实现为新的 AgentRun，并用 Planner/Reviewer call counter 同时承担技术重试和语义调用限制；Server 重启后，未完成的 Roadmap Run 也没有按统一契约标记为 `Interrupted`。

**目标行为：** 一次语义 round 中，每个角色的一次逻辑调用对应一个 AgentRun；同一 AgentRun 内最多执行五次技术 attempt。语义拒绝、技术重试和中断替代必须是三种独立含义。

**实现边界：**

- Planner 的一次规划和每个 Reviewer 的一次审查分别拥有一个 AgentRun；模型传输、工具调用或 typed-result 校验等技术错误只递增该 Run 的 `attempt`，最多五次。
- Reviewer reject 是成功持久化的语义结果，不是技术失败；它完成当前 Run，并在需要修订时进入下一 semantic round，创建新的 Planner/Reviewer AgentRun。
- 现有 role call counter 只限制 semantic call，不计算同一 Run 内的技术 attempt。
- Server 启动时统一把未完成的 Roadmap AgentRun 标记为 `Interrupted`、释放旧 task claim，并从已持久化边界创建替代 Run：未完成 Planner 时只替代 Planner，Planner 结果已持久化时只替代尚未完成的 Reviewer。
- 中断替代不增加 semantic round 或 role call counter，不恢复未完成模型上下文或工具执行。

**必须验证：** 单个 Roadmap Run 的 attempt 单调递增且不超过五次；reject 正确开启新 round；技术失败不消耗 semantic call counter；重启后旧 Run 为 `Interrupted`，已持久化结果不重做，替代 Run 不改变 round 与 call counter。

### 6. 清理过时的 Skill 与 Taxonomy 设计

**现状问题：** `NEXT.md` 仍使用 Skill、TaxonomyWorkflow、mapping 和 taxonomy snapshot 描述未来题库，与当前 `Domain -> Topic -> Challenge`、Classification 和 RoadmapWorkflow 模型冲突。

**目标行为：** 仓库只保留一套当前领域语言和工作流边界，后续内容建设不会再以已移除的 Skill/Taxonomy 方案为依据。

**实现边界：**

- 删除 `NEXT.md` 中已经完成或失效的 Skill/Taxonomy 方案。
- 用 Domain、Topic、Challenge、Classification 和 Roadmap 的当前术语重写仍然有效的产品方向，并链接 `docs/architecture/` 中的正式契约。
- P0 实现完成后从 `TODO.md` 删除已完成项；不把历史迁移方案继续留作第二份架构文档，也不保留兼容术语。

**必须验证：** 面向当前设计的文档不再把 Skill、TaxonomyWorkflow、mapping 或 taxonomy snapshot 表述为有效模型；文档链接与代码、API 和持久化术语一致。

### 实施顺序与完成标准

1. 先实现 VK8sEnvironment 网络隔离和调试路由安全边界，关闭当前可直接利用的控制面风险。
2. 实现 `materialized_revision`、严格 Catalog 投影和 bootstrap-safe readiness，建立发布内容的完整性边界。
3. 拆分 Runtime Worker 的 Action/Reaper 循环，并修正 Generation/Catalog 的轮换领取。
4. 统一 Roadmap AgentRun 的 attempt、semantic round、call counter 与启动恢复语义。
5. 清理 `NEXT.md` 和正式架构文档中的过时设计。
6. 完成聚焦单元/集成验证，并在执行 NetworkPolicy 的真实 CNI 上完成 VK8s 隔离验证；最后运行仓库现有的完整 Go、前端、生成物和部署清单检查。

### P0 提交计划

P0 按以下六个原子提交实施。每一项的实现、对应测试、生成物和必要部署/架构文档必须在该项验证通过后一起提交，再开始下一项；不能先堆积多项修改，也不能把测试集中到最后形成单独的验证提交。提交信息可以在实施时按仓库实际改动微调，但不能改变提交边界。

1. **`fix(vk8s): enforce learner network isolation`**
   - **目标：** 启用 vcluster 原生 control-plane/workload NetworkPolicy，补充 management terminal 和部署环境特定受保护 CIDR 的策略与配置，形成 VK8sEnvironment 默认网络边界。
   - **验证：** 配置和资源渲染覆盖所有组件且不写死开发集群网络；题目内部通信、DNS 和允许的公网出口正常；在执行 NetworkPolicy 的真实 CNI 上确认学习者 workload 无法访问平台、宿主 API、私网和 metadata。
2. **`fix(debug): secure and disable debug routes by default`**
   - **目标：** 默认不注册 Roadmap maintenance/export 调试路由；显式开启时接入独立 debug credential，并同步收紧部署配置和路由边界。
   - **验证：** 默认部署不存在可调用的调试入口；开启后只有 debug credential 有权调用，用户 JWT 与 Runtime Worker identity 均不能复用；maintenance 和 export 的合法调试流程可用。
3. **`fix(catalog): enforce materialized catalog integrity`**
   - **目标：** 定义并持久化 `materialized_revision`，让作者发布和 Catalog Release 共享相同计算规则；实现 Roadmap binding 到物化文件的严格投影，以及绕过 availability gate 的 bootstrap-safe readiness 检查。
   - **验证：** 被引用题目的目录、内容、路径或可执行位发生缺失/变化时均返回完整性错误；额外未引用目录不破坏正常发布窗口；空平台可完成首个 Release；portable release 不携带平台完整性字段。
4. **`fix(runtime): run action and reaper loops independently`**
   - **目标：** 在同一个 Runtime Worker 进程中独立运行 Action Loop 与 Reaper Loop，并让 Server 的 action/cleanup claim 分别在 Generation 与 Catalog 之间公平轮换，不增加新队列、Deployment 或状态。
   - **验证：** 普通 action 持续积压时 cleanup 仍能推进；Generation/Catalog 任一侧持续有任务时另一侧不会饥饿；现有 lease、接管、迟到结果拒绝与幂等清理继续成立。
5. **`fix(roadmap): align roadmap execution with AgentRun contract`**
   - **目标：** 将一个 semantic role call 固定为一个 AgentRun，在 Run 内实现最多五次 technical attempt；分离 reviewer reject、semantic round、role call counter 与启动中断替代语义。
   - **验证：** 技术失败只递增当前 Run attempt，reject 才开启下一 round；技术重试不消耗 role call counter；Server 重启后旧 Run 成为 `Interrupted`，只替代未完成角色，且不重复已持久化结果或改变 round/counter。
6. **`docs: remove obsolete skill and taxonomy design`**
   - **目标：** 删除 `NEXT.md` 中失效的 Skill/Taxonomy 方案，以 Domain、Topic、Challenge、Classification 和 Roadmap 统一当前文档，并在实现完成后移除已完成的 P0 待办。
   - **验证：** 当前文档、代码、API 和持久化术语一致，不再存在仍被描述为有效模型的 Skill/Taxonomy 设计；完成全仓 Go、前端、生成物、部署清单和聚焦集成回归检查。

P0 完成标准是：VK8s 学习者无法访问平台网络；调试接口默认不存在且拥有独立身份；Roadmap 与物化题库的任何缺失或篡改都能被读取和 readiness 发现；普通 runtime action 无法饿死资源清理，Generation/Catalog 无固定偏序；Roadmap 完整遵守统一 AgentRun 契约；仓库不再同时维护 Skill/Taxonomy 与 Domain/Topic/Roadmap 两套设计。
