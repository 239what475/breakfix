...
根目录入口和历史命名已在 P1/P2 后完成整理：根 `README.md` 指向权威配置、
Makefile 与稳定文档；测试辅助函数不再使用过时的 `Gateway` 名称；随 P2 删除源码
留下的空 `internal/ca` 与 `internal/proxy` 目录也已移除。这些维护项不再是开放风险。
